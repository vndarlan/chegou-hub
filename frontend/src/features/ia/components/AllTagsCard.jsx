...
export default function AllTagsCard({ configId, onRefresh }) {
  ...
  // Mostrar TODAS as tags sem filtro
      ...
      <CardContent>
        ...
        {loading && tagsFiltradas.length === 0 ? (
          ...
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-sm mt-1">Os contatos ainda não possuem tags</p>
          </div>
        ) : (
          <div className="space-y-1">
            {/* Informação do total */}
            <div className="text-xs text-muted-foreground mb-4 pb-3 border-b">
              {tagsFiltradas.length} tags de {totalSubscribers.toLocaleString('pt-BR')} contatos
            </div>
